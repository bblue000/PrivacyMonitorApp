...
public class PersonController extends BaseController {
	...
	public void requestLoactionData(LoactionDataLoadListener listener) {
		...
		aQuery.ajax(
				String.format(Config.URL_GET_LOCATION,
						mMonitoredPerson.getDevice_token(),
						mMonitoredPerson.getNote_name(),
						AndroidUtils.getDeviceId()), RespLocationResult.class,
				mAjaxCallback);
	}
	...
	private void calculate() {
		...
		for (int i = 0; i < mLocationInfoList.size(); i++) {
			...
			if (lastDay != curDay) {
				lastDay = curDay;
				mDateTimeData.put(curDay * ONEDAY,
						curList = new ArrayList<RespLocation>());
			} else {
				curList.add(info);
			}
		}
		LogUtils.d(TAG,
				"calculate mDateTimeData.size() = " + mDateTimeData.size());
	}
	...
	private class AjaxCallbackImpl extends AjaxCallback<RespLocationResult> {
		...
		public void setLoactionDataLoadListener(
				LoactionDataLoadListener loactionDataLoadListener) {
			mLoactionDataLoadListener = loactionDataLoadListener;
		}

		@Override
		public void callback(String url, RespLocationResult result,
				AjaxStatus status) {
			...
			if (null != mLoactionDataLoadListener) {
				if (status.getCode() == HttpStatus.SC_OK) {
					try {
						...
						e.printStackTrace();
						ToastUtils
								.showToast(R.string.persion_location_data_obtain_error);
						mLoactionDataLoadListener.onLocationLoadFailed();
					}
